#!/bin/bash
echo "🔧 Creando motor_clinico_v3.js..."

cat << 'EOT' > motor_clinico_v3.js
// motor_clinico_v3.js
...
if [ -f "core_zara_v5.js" ]; then
  echo "Integrando motor_clinico_v3.js..."
  sed -i '' '/motor_clinico_v2/d' core_zara_v5.js
  if ! grep -q "motor_clinico_v3" core_zara_v5.js; then
    echo 'const { clasificarPlan } = require("./motor_clinico_v3");' | cat - core_zara_v5.js > temp && mv temp core_zara_v5.js
  fi
fi

git add motor_clinico_v3.js core_zara_v5.js
git commit -m "v8.1 motor_clinico_v3 integrado correctamente"
git push origin main
echo "🚀 Deploy enviado a Render. Verifica logs."
