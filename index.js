...
app.get("/panel", (req, res) => {
  try {
    ...
    <style>
      body {font-family: Arial; background: #fafafa; padding: 20px;}
      .msg {background: #fff; margin-bottom: 10px; padding: 10px; border-radius: 8px; box-shadow: 0 0 5px #ccc;}
      .numero {font-weight: bold; color: #222;}
      .fecha {color: #777; font-size: 12px;}
    </style></head>
    <body><h1>Conversaciones Zara Body Elite</h1>
    ${data.map(m=>`<div class='msg'><div class='numero'>${m.numero}</div><div>${m.mensaje}</div><div class='fecha'>${m.fecha}</div></div>`).join("")}
    </body></html>`;
    res.send(html);
  } catch (e) {
