...
<body>
    ...
    <div id="rp" class="mod">
        ...
        <div style="display:flex;gap:10px;justify-content:center;margin-bottom:20px;align-items:center">
            <span>Desde:</span> <input type="date" id="d-start" class="d-picker" onchange="calcStats()">
            <span>Hasta:</span> <input type="date" id="d-end" class="d-picker" onchange="calcStats()">
        </div>
        <div style="background:#202c33;padding:15px;border-radius:8px;text-align:center;margin-top:15px;border:1px solid #374045;">
            <div style="font-size:0.9em;color:#8696a0">TOTAL LEADS</div>
        ...
        function renderList(st) {
            ...
            Object.keys(allUsers).sort((a,b)=>allUsers[b].lastInteraction-allUsers[a].lastInteraction).forEach(p=>{
                ...
                l.innerHTML += '<div class="cd '+(ap===p?'active':'')+'" onclick="sl(\\''+p+'\\')">' +
                    '<div style="display:flex;justify-content:space-between;margin-bottom:4px"><span style="font-weight:600">'+u.name+'</span><span style="font-size:0.7em;color:#8696a0">'+new Date(u.lastInteraction).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})+'</span></div>' +
