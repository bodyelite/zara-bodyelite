...
// SCRIPTS DEL NAVEGADOR (Separados para evitar errores)
const CLIENT_SCRIPT = `
...
async function tg() {
    ...
    try {
        ...
        up(); // Actualizar inmediatamente
    } catch(e) { alert("Error al cambiar estado"); }
}

async function st() {
    await fetch("/api/tag",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({phone:ap,tag:document.getElementById("ts").value})});
    up();
}

async function rn(t) {
    if(!confirm("¿Lanzar estrategia?")) return;
    await fetch("/api/estrat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tag:t})});
    up();
}

function setFilter(n, b) { 
    fl=n; 
    document.querySelectorAll('.filters .f-btn').forEach(x=>x.classList.remove('active')); 
...
function renderList(st){
    ...
    Object.keys(allUsers).sort((a,b)=>allUsers[b].lastInteraction-allUsers[a].lastInteraction).forEach(p=>{
        ...
        l.innerHTML+='<div class="cd '+(ap===p?'active':'')+'" onclick="sl(\\''+p+'\\')"><div style="display:flex;justify-content:space-between;margin-bottom:4px"><span style="font-weight:600">'+u.name+'</span><span style="font-size:0.7em;color:#8696a0">'+new Date(u.lastInteraction).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})+'</span></div><div style="display:flex;justify-content:space-between;align-items:center"><span style="font-size:0.75em;color:#8696a0">'+f(u.lastInteraction)+'</span><span class="tag tag-'+u.tag+'">'+u.tag+'</span></div></div>';
    });
}
...
function rd(u){
    ...
    const html=u.history.map(m=>{
        ...
        return '<div class="msg '+type+'"><div>'+m.content+'</div><div style="font-size:0.65em;opacity:0.6;text-align:right;margin-top:4px">'+f(m.timestamp)+'</div></div>';
    }).join("");
...
async function sd(){
    ...
    if(!v.value)return;
    await fetch("/api/manual",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({phone:ap,text:v.value})});
    v.value="";
    up();
}
...
app.post('/api/reservo', async (req, res) => {
    console.log("📨 RESERVO RAW:", req.body);
