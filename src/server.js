...
app.get('/monitor', (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            body { margin: 0; font-family: system-ui, sans-serif; background: var(--bg); color: var(--text); display: flex; height: 100vh; overflow: hidden; font-size: 13px; }
            ...
            .lead-card { padding: 12px; border-bottom: 1px solid #1e293b; cursor: pointer; }
    ...
    <body>
    <div class="sidebar">
        <div style="padding:15px; font-weight:800; color:var(--primary); font-size:16px;">ZARA CRM 9.0</div>
        <div class="tabs-grid">
    ...
    <div class="main">
        ...
        <div style="padding:15px; background:#1e293b; display:flex; gap:10px">
            <input type="text" id="msgIn" style="flex:1; padding:10px; border-radius:8px; border:none; background:#0f172a; color:white">
            <button onclick="sendManual()" style="padding:10px 20px; background:var(--primary); color:white; border:none; border-radius:8px; cursor:pointer">ENVIAR</button>
        </div>
    </div>
    <script>
        ...
        function selectLead(p){ curPhone=p; renderChat(); renderList(); }
        function renderChat(){
