...
app.get('/monitor', (req, res) => {
    ...
    <style>
        :root { --bg: #0b141a; --sidebar: #111b21; --accent: #00a884; --hot: #e91e63; --int: #ff9800; --frio: #667781; --auto: #a855f7; } 
        ...
        .dash-header { padding: 15px; background: #202c33; border-bottom: 1px solid #2a3942; text-align:center; font-weight:bold; color:var(--accent); }
        ...
        .c-tag { font-size: 0.6em; padding: 2px 6px; border-radius: 4px; font-weight: 800; color:white; }
        ...
        .msg { max-width: 80%; padding: 10px 14px; border-radius: 8px; font-size: 0.9em; position: relative; line-height: 1.4; }
        ...
        @media (max-width: 768px) {
            ...
            .sidebar-left { width: 100%; height: 35vh; }
    ...
    <body>
        <div class="sidebar-left">
            <div class="dash-header">ZARA PANEL</div>
            <div class="nav-tabs"><button class="tab active" onclick="setFilter('ALL')">TODOS</button><button class="tab" onclick="setFilter('HOT')">HOT 🔥</button></div>
            <div id="list"></div>
        </div>
        <div class="main-chat">
            <div id="feed"><div style="text-align:center;margin-top:20%;opacity:0.3">Selecciona un cliente</div></div>
            ...
            function update(){ 
                fetch("/api/data").then(r=>r.json()).then(d=>{
                    ...
                    Object.keys(users).sort((a,b)=>(users[b].lastInteraction||0)-(users[a].lastInteraction||0)).forEach(p => {
                        const u = users[p]; if(filter!=='ALL' && u.tag!==filter) return;
                        l.innerHTML += \`<div class="card \${u.tag} \${ap===p?'active':''}" onclick="select('\${p}')">
                            ...
                            <div style="display:flex;justify-content:space-between;align-items:center"><span style="font-size:0.7em;opacity:0.6">\${fmt(u.lastInteraction)}</span><span class="c-tag" style="background:var(--\${u.tag==='HOT'?'hot':u.tag==='INTERESADO'?'int':'frio'})">\${u.tag}</span></div>
                        </div>\`;
                    });
