...
app.get('/monitor', (req, res) => {
    ...
    html += '.tag.CALIENTE { background: var(--hot); color: white; } .tag.TIBIO { background: var(--medium); } .tag.FRIO { background: var(--cold); } .tag.NUEVO { background: #666; color: white; }';
    ...
    html += '<div class="header"><span class="brand">ZARA 7.0</span><button class="btn-csv" onclick="alert(\'CSV Descargado\')">📥 CSV</button></div>';
    ...
    html += 'function renderList() { const list = document.getElementById("list"); list.innerHTML = ""; Object.keys(users).forEach(phone => { const u = users[phone]; if(!u) return; const history = u.history || []; const last = history.length > 0 ? history[history.length-1] : { content: "Sin mensajes" }; ';
    ...
    html += 'div.innerHTML = `<div class="avatar">${(u.name && u.name[0]) ? u.name[0] : "?"}</div><div class="info"><div class="name"><span>${u.name || "Cliente"}</span><span class="tag ${tagClass}">${u.tag || "NUEVO"}</span></div><div class="last-msg">${last.content}</div></div>`; list.prepend(div); }); }';
    ...
    html += 'function renderChat(phone) { const feed = document.getElementById("feed"); feed.innerHTML = ""; if(users[phone] && users[phone].history) { users[phone].history.forEach(msg => { const div = document.createElement("div"); div.className = "msg " + msg.role; div.innerHTML = msg.content; feed.appendChild(div); }); } feed.scrollTop = feed.scrollHeight; }';
...
app.listen(PORT, () => console.log(`🟢 ZARA 7.0 ONLINE en puerto ${PORT}`));
