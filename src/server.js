...
<div style="display:flex;gap:10px;align-items:center"><button id="bt" class="btn" onclick="tg()"></button>
<select id="ts" onchange="st()" style="background:#2a3942;color:#fff;border:1px solid #444;padding:4px;border-radius:4px">
...
<div id="fd"></div>
<div class="ib" id="i" style="display:none"><input id="m" placeholder="Respuesta manual..." onkeypress="if(event.key==='Enter')sd()"></div>
</div>
<div class="sr"><button class="btn" style="background:var(--ht)" onclick="rn('HOT')">CIERRE HOT 🔥</button>
<button class="btn" style="background:var(--mn)" onclick="op()">REPORTES / FUNNEL</button>
<button class="btn" style="background:#444;margin-top:10px" onclick="location.reload()">REFRESCAR</button></div>
<div id="rp" class="mod"><h2>Funnel de Leads</h2><div id="fn" style="display:flex;gap:10px;margin-top:20px"></div><button class="btn" style="background:var(--ht);margin-top:40px" onclick="document.getElementById('rp').style.display='none'">CERRAR</button></div>
<script>
let ap=null;let fl='ALL';const f=(t)=>new Date(t).toLocaleString('es-CL',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'});
...
function rd(u){const d=document.getElementById("fd");d.innerHTML=u.history.map(m=>'<div class="msg '+(m.role==='user'?'user':(m.source==='manual'?'manual':'bot'))+'"><div>'+m.content+'</div><div style="font-size:0.6em;opacity:0.5;text-align:right;margin-top:4px">'+f(m.timestamp)+'</div></div>').join("");d.scrollTop=d.scrollHeight;}
