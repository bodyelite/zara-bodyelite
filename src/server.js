...
app.get('/monitor', (req, res) => {
    ...
    <style>
        ...
        .chat-header { padding: 10px 20px; background: #202c33; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #2a3942; }
        ...
        select.tag-picker { background: #2a3942; color: white; border: 1px solid #333; padding: 5px; border-radius: 4px; font-size: 0.8em; }
    ...
    <body>
        ...
        <div class="main-chat">
            <div class="chat-header" id="c-head" style="display:none">
                <span id="active-name" style="font-weight:bold"></span>
                <select class="tag-picker" id="tag-sel" onchange="changeTag()">
                    <option value="NUEVO">NUEVO</option>
                    <option value="HOT">HOT 🔥</option>
                    <option value="INTERESADO">INTERESADO 🔸</option>
                    <option value="FRIO">FRIO ❄️</option>
                    <option value="APAGADO">APAGADO 🗑️</option>
                </select>
            </div>
            <div id="feed"></div>
        ...
        <script>
            ...
            async function changeTag() {
                const nt = document.getElementById("tag-sel").value;
                await fetch("/api/tag",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({phone:ap,tag:nt})});
                update();
            }
            ...
            function update(){ 
                fetch("/api/data").then(r=>r.json()).then(d=>{
                    ...
                    Object.keys(users).sort((a,b)=>(users[b].lastInteraction||0)-(users[a].lastInteraction||0)).forEach(p => {
                        const u = users[p];
                        l.innerHTML += \`<div class="card \${ap===p?'active':''} \${u.tag}" onclick="select('\${p}')">
                            <div class="c-header"><span class="c-name">\${u.name}</span><span class="c-time">\${fmt(u.lastInteraction)}</span></div>
                            <div style="font-size:0.7em;opacity:0.6">\${u.tag}</div>
                        </div>\`;
                    });
                    if(ap && users[ap]) {
                        document.getElementById("active-name").innerText = users[ap].name;
                        document.getElementById("tag-sel").value = users[ap].tag;
                        renderChat(users[ap]);
                    }
                });
            }
