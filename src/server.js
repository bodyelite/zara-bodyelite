...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, system-ui, sans-serif; }
        ...
        .tab.active { color: white; transform: scale(1.02); }
...
<body>
    ...
    <div class="main">
        ...
        <div class="input-area" id="input-area" style="display:none">
            <input type="text" id="msg-input" placeholder="Escribe aquí..." onkeydown="if(event.key==='Enter') send()">
...
<script>
    ...
    function renderList() {
        ...
        sorted.forEach(id => {
            ...
            let color = '#777'; let ico = 'fa-user';
            if(c.origen === 'whatsapp') { color = '#25D366'; ico = 'fa-whatsapp'; }
            if(c.origen === 'instagram') { color = '#E1306C'; ico = 'fa-instagram'; }
            if(c.origen === 'web') { color = '#007bff'; ico = 'fa-globe'; }

            const last = c.mensajes.slice(-1)[0] || {};
            html += \`<div class="contact \${curId===id?'active':''}" onclick="openChat('\${id}', '\${c.origen}')">
                <div class="avatar" style="background:\${color}">
                    <i class="fab \${ico}"></i>
                    <div class="badge" style="color:\${color}"><i class="fas \${ico}"></i></div>
                </div>
                <div class="details">
    ...
    function openChat(id, origin) {
        curId = id; curOrigin = origin;
        document.getElementById('header').innerText = data[id].nombre;
