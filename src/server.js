...
app.get('/monitor', (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            * { box-sizing: border-box; }
            ...
            .checkbox-area { width: 30px; height: 100%; display: flex; align-items: center; justify-content: center; cursor: pointer; padding-left:10px; }
            .lead-checkbox { width: 16px; height: 16px; cursor: pointer; accent-color: var(--primary); pointer-events: none; }
            
            .lead-info { flex:1; padding: 0 10px; height: 100%; display: flex; flex-direction: column; justify-content: center; cursor: pointer; }
            ...
            .call-btn-list { width: 30px; height: 30px; display:flex; align-items:center; justify-content:center; background: #f0fdf4; border-radius: 50%; color: #166534; text-decoration: none; margin-right: 10px; border: 1px solid #bbf7d0; transition: all 0.2s; }
            ...
            /* Barra Masiva */
            .bulk-toolbar { position: absolute; bottom: 30px; left: 50%; transform: translateX(-50%); background: #111827; color: white; padding: 10px 20px; border-radius: 40px; display: none; align-items: center; gap: 15px; z-index: 100; box-shadow: 0 10px 25px rgba(0,0,0,0.3); }
            ...
            .fixed-note { font-size: 12px; color: #92400e; margin-bottom: 5px; display: flex; gap: 8px; align-items: flex-start; }
            .fixed-note-icon { font-size: 14px; }
            .fixed-note-time { font-weight: 700; font-size: 10px; color: #b45309; min-width: 80px; }
            ...
            .select-all-btn { width: 100%; padding: 5px; margin-bottom: 5px; border: 1px dashed #cbd5e1; background: #f8fafc; color: #64748b; font-size: 10px; cursor: pointer; border-radius: 4px; }
    ...
    <div class="tools">
        ...
        <div>
            <label style="font-size:11px; font-weight:700; color:#6b7280;">PROGRAMAR TAREA (BITÁCORA)</label>
            ...
            <button class="btn btn-outline" style="margin-top:5px;" onclick="addNote()">Guardar en Bitácora</button>
        </div>
    ...
    <script>
        ...
        function renderChat(){
            ...
            // RENDERIZAR NOTAS EN PANEL FIJO SUPERIOR
            const notesPanel = document.getElementById('stickyNotesPanel');
            if(u.notes && u.notes.length > 0) {
                let notesHtml = '';
                u.notes.forEach(n => {
                    const created = new Date(n.date).toLocaleDateString([], {day:'2-digit', month:'2-digit'});
                    let extra = '';
                    if(n.scheduleTime) {
                        const s = new Date(n.scheduleTime);
                        extra = \`<span style="background:#fef3c7; padding:2px 5px; border-radius:4px; margin-left:5px;">⏰ \${s.getDate()}/\${s.getMonth()+1} \${s.getHours()}:\${String(s.getMinutes()).padStart(2,'0')}</span>\`;
                    }
                    notesHtml += \`
                    <div class="fixed-note">
                        <span class="fixed-note-icon">📝</span>
                        <span class="fixed-note-time">\${created}</span>
                        <span>\${n.text} \${extra}</span>
                    </div>\`;
                });
            ...
            // RENDERIZAR MENSAJES (Sin notas mezcladas)
            let html='';
            (u.history||[]).forEach(m=>{ 
