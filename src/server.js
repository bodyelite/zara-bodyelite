...
.mod{position:fixed;top:10%;left:10%;width:80%;height:80%;background:#202c33;display:none;padding:30px;border-radius:10px;z-index:100;}
...
<div class="mc">
<div class="ch" id="h" style="display:none"><div><b id="n"></b> <small id="p" style="color:#8696a0"></small></div>
<div style="display:flex;gap:8px"><button id="bt" class="btn" onclick="tg()"></button>
<select id="ts" onchange="st()" style="background:#2a3942;color:#fff;border:1px solid #444;border-radius:4px">
...
<div class="sr"><button class="btn" style="background:var(--ht)" onclick="rn('HOT')">CIERRE 🔥</button>
<button class="btn" style="background:var(--mn)" onclick="op()">REPORTES</button>
<button class="btn" style="background:#444" onclick="location.reload()">REFRESCAR</button></div>
<div id="rp" class="mod"><h2>Funnel Zara</h2><div id="fn" style="display:flex;gap:10px"></div><button class="btn" style="background:var(--ht);margin-top:20px" onclick="document.getElementById('rp').style.display='none'">CERRAR</button></div>
<script>
...
function op(){fetch("/api/data").then(r=>r.json()).then(d=>{const c={NUEVO:0,INTERESADO:0,HOT:0,APAGADO:0};Object.values(d.users).forEach(u=>c[u.tag]++);document.getElementById("fn").innerHTML=\`<div style="flex:1;background:var(--ac);padding:10px">NUEVOS: \${c.NUEVO}</div><div style="flex:1;background:var(--it);padding:10px">INT: \${c.INTERESADO}</div><div style="flex:1;background:var(--ht);padding:10px">HOT: \${c.HOT}</div>\`;document.getElementById("rp").style.display='block';});}
