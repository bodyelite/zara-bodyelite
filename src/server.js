...
// MONITOR HTML PURO Y SIMPLE (Sin complicaciones)
app.get('/monitor', (req, res) => {
res.send(`<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ZARA 10.1</title>
<style>
...
function renderList(st){const l=document.getElementById("list");l.innerHTML="";Object.keys(allUsers).sort((a,b)=>allUsers[b].lastInteraction-allUsers[a].lastInteraction).forEach(p=>{const u=allUsers[p];if(fl!=='ALL'&&u.tag!==fl)return;l.innerHTML+='<div class="cd '+(ap===p?'active':'')+'" onclick="sl(\\''+p+'\\')"><div style="display:flex;justify-content:space-between;margin-bottom:4px"><span style="font-weight:600">'+u.name+'</span><span style="font-size:0.7em;color:#8696a0">'+new Date(u.lastInteraction).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})+'</span></div><div style="display:flex;justify-content:space-between;align-items:center"><span style="font-size:0.75em;color:#8696a0">'+f(u.lastInteraction)+'</span><span class="tag tag-'+u.tag+'">'+u.tag+'</span></div></div>';});}
function sl(p){ap=p;document.getElementById("h").style.display="flex";document.getElementById("i").style.display="flex";up();}function updateChatUI(u,st){document.getElementById("n").innerText=u.name;document.getElementById("p").innerText=u.phone;document.getElementById("ts").value=u.tag;const b=document.getElementById("bt");const isOff=st[u.phone]===false;b.innerText=isOff?'BOT OFF 🔴':'BOT ON 🟢';b.style.background=isOff?'#333':'var(--ac)';rd(u);}
