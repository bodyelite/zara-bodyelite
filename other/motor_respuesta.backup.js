...
export function responderExtendido(textoUsuario) {
  if (obj) return obj;
  // llamada limpia a bloque de objeción de precio
  if (obj) return obj;
  if (obj) return obj;
  if (obj) return obj;
  // Prioridad 1: empatía
...
// --- Respuestas ante objeciones de precio o comparación ---
  const t = texto.toLowerCase();
  if (t.includes("caro") || t.includes("más barato") || t.includes("carísima") || t.includes("carisimo")) {
  }
  return null;
}

// Integrar en flujo extendido
  const t = texto.toLowerCase();

  if (t.includes("caro") || t.includes("más barato") || t.includes("carísima") || t.includes("carisimo")) {
    return "💎 Entiendo tu punto. Nuestros tratamientos usan equipos clínicos de alta tecnología (HIFU 12D, RF, EMS Sculptor) con resultados visibles desde la primera sesión y seguimiento profesional. La evaluación inicial es gratuita para que confirmes el valor por ti misma.";
  }

  if (t.includes("vale la pena") || t.includes("por qué ese precio") || t.includes("cuesta tanto")) {
    return "🌟 Los valores reflejan la tecnología y la seguridad que ofrecemos. Todo incluye diagnóstico con IA, control médico estético y resultados reales sin cirugía. Muchas pacientes nos eligen por eso.";
  }
